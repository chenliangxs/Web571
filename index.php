...
foreach($types as $type){
   if($type=="place"){
       $searchURLUser='https://graph.facebook.com/v2.8/search?q='.$keyword.'&type='.$type.'&center='.$latitude.','.$longitude.'&distance='.'1000'.'&fields=id,name,picture.width(700).height(700)&access_token='.$my_access_token;
   }
    else{
        $searchURLUser='https://graph.facebook.com/v2.8/search?q='.$keyword.'&type='.$type.'&fields=id,name,picture.width(700).height(700)&access_token='.$my_access_token;
    }
